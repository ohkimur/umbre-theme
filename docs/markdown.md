...
| Preview diffs                      | `src/markdown/diff-markers.ts` keeps VS Code's change markers out of Markdown syntax and code, so changed text stays highlighted           |
...
| Markdown editor      | Directory trees and code are colored by the editor's tokenizer                                               | Previews color code with highlight.js, mapped to the same syntax roles                                                                                       |
| Markdown editor diff | Added blocks show as raw Markdown source                                                                     | Diff rendering                                                                                                                                               |
| Preview diffs        | Changes inside a Mermaid diagram are not highlighted, and changed front matter shows without highlights      | The diagram replaces its source text, and front matter renders as a table, so there is no changed text left to mark                                          |
| Preview diffs        | Marker markup written into a code example, such as `<span data-diff-start="0"></span>`, can show as a change | Inside a diff, VS Code's markers and authored ones look the same; outside a diff, examples render as written                                                 |
